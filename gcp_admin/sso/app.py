...
# --- NEW: Helper function to generate the guaranteed HTTPS redirect URI ---
def get_redirect_uri():
    """Generates the external, HTTPS-enforced redirect URI for Google OAuth."""
    # The _external=True generates the full URL (domain + path)
    # The _scheme='https' is CRITICAL to force HTTPS protocol for Cloud Run
...
def login():
    ...
    # Use the helper function to get the guaranteed HTTPS redirect URI
...
def callback():
    ...
    # Get the guaranteed HTTPS redirect URI for the token exchange step
    redirect_uri = get_redirect_uri()
    ...
    token_response = requests.post(
        ...
        data={
            ...
            "redirect_uri": redirect_uri, # Must match the URL used in the /login step
